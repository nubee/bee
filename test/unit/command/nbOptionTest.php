...
try {
  $foo = new nbOption('foo','f',nbOption::IS_ARRAY,'foo description');
  $t->fail('mode couldn\'t be only IS_ARRAY');
}
catch(InvalidArgumentException $e)
{
  $t->pass('mode couldn\'t be only IS_ARRAY');
}

try {
  $foo = new nbOption('foo','f',nbOption::PARAMETER_NONE | nbOption::IS_ARRAY,'foo description');
  $t->fail('option couldn\'t have PARAMETER_NONE and be an array');
}
catch(InvalidArgumentException $e)
{
  $t->pass('option couldn\'t have PARAMETER_NONE and be an array');
}
