...
TXT
  ...
  protected function execute(array $arguments = array(), array $options = array()) {
    ...
    $buildVersionFile = nbConfig::get('project_repository_root-directory') . "/version.yml";

    if (nbConfig::has('project_repository') and nbConfig::get('project_repository_type') == 'git') {
      ...
      if ($code == 0 and !$this->askConfirm(sprintf('Error in "%s", do you want to continue anyway?', $cmd))) {
        $this->logLine('Bye');
        die;
      }
      //- git status
      ...
      if ($interactiveMode and !$this->askConfirm("Do you want to commit your changes to your local repository?")) {
        $this->logLine('Bye');
        die;
      }
      ...
      //- git pull
      if ($interactiveMode and !$this->askConfirm("Do you want to update from your remote repository?")) {
        $this->logLine('Bye');
        die;
      }
      ...
      //- bee test:all
      if (!$interactiveMode or ($interactiveMode and $this->askConfirm("Do you want to test your project?"))) {
        ...
        if (($code == 0 and !$interactiveMode and !$forceMode) or ($code == 0 and $interactiveMode and  !$this->askConfirm('There are some errors in your tests, do you want to continue anyway?'))){
          $this->logLine('Bye');
          die;
        }
      }
      ...
      if ($interactiveMode and !$this->askConfirm("Do you want to commit your changhes in the remote repository?")) {
        $this->logLine('Bye');
        die;
      }
  ...
  private function askConfirm($message) {
